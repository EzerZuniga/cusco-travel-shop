...
class TurismoCuscoApp {
    ...
    loadInitialData() {
        ...
        // Soportar rutas antiguas (*.html) y rutas nuevas en Laravel (/tours, /blog, /gallery)
        if ((path.includes('tours.html') || path.includes('/tours')) && window.ToursManager) {
            window.ToursManager.loadTours();
        }

        if ((path.includes('blog.html') || path.includes('/blog')) && window.BlogManager) {
            window.BlogManager.loadPosts();
        }
        
        if ((path.includes('gallery.html') || path.includes('/gallery')) && window.GalleryManager) {
